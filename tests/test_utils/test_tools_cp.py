...
class TestRemoveSmallObjectsCp(unittest.TestCase):
    ...
    def test_remove_small_objects_boolean_input(self, mock_label, mock_gen_struct, 
                                              mock_bincount, mock_zeros_like):
    ...
    @patch('cupyx.scipy.ndimage.generate_binary_structure')
    @patch('cupyx.scipy.ndimage.label')
    def test_remove_small_objects_realistic_case(self, mock_label, mock_gen_struct, 
                                               mock_bincount, mock_zeros_like):
        ...
        mock_label.side_effect = lambda input_array, structure, output: setattr(output, 'ravel', lambda: np.array([0, 1, 1, 1, 2, 2, 3, 3, 3]))
