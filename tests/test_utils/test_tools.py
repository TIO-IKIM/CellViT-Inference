...
class TestGetBoundingBox(unittest.TestCase):
    ...
    def test_get_bounding_box_single_pixel(self):
        ...
        result = get_bounding_box(img)
        self.assertEqual(
            result, [4, 5, 5, 6], "Bounding box for single pixel should be [4, 5, 5, 6]"
        )
    ...
    def test_get_bounding_box_rectangle(self):
        ...
        result = get_bounding_box(img)
        self.assertEqual(
            result, [2, 6, 3, 8], "Bounding box for rectangle should be [2, 6, 3, 8]"
        )
...
class TestRemoveSmallObjects(unittest.TestCase):
    ...
    def test_remove_small_objects_no_removal(self):
        ...
        np.testing.assert_array_equal(
            result, pred, "No objects should be removed when all are above min_size"
        )
    ...
    def test_remove_small_objects_removal(self):
        ...
        expected[5:9, 5:9] = 2
        np.testing.assert_array_equal(
            result, expected, "Small objects should be removed"
        )
    ...
    def test_remove_small_objects_all_removed(self):
        ...
        expected = np.zeros((10, 10), dtype=int)
        np.testing.assert_array_equal(
            result, expected, "All objects should be removed when below min_size"
        )
    ...
    def test_remove_small_objects_binary_input(self):
        ...
        np.testing.assert_array_equal(
            result, expected, "Small objects should be removed for binary input"
        )
    ...
    def test_remove_small_objects_no_min_size(self):
        ...
        np.testing.assert_array_equal(
            result, pred, "No objects should be removed when min_size is 0"
        )
...
class TestGetSizeOfDict(unittest.TestCase):
    ...
    def test_get_size_of_dict_multiple_key_values(self):
        ...
        d = {"a": 1, "b": 2, "c": 3}
        expected_size = sys.getsizeof(d) + sum(
...
class TestLoadWsiFilesFromCsv(unittest.TestCase):
    ...
    def test_load_wsi_files_from_csv_valid_extension(self):
        ...
        self.assertEqual(
            result, expected, "Should return files with the specified extension"
        )
    ...
    def test_load_wsi_files_from_csv_invalid_extension(self):
        ...
        expected = []
        self.assertEqual(
            result, expected, "Should return an empty list for an invalid extension"
        )
    ...
    def test_load_wsi_files_from_csv_empty_csv(self):
        ...
        expected = []
        self.assertEqual(
            result, expected, "Should return an empty list for an empty CSV"
        )
    ...
    def test_load_wsi_files_from_csv_missing_column(self):
        ...
        pd.DataFrame({"NotFilename": ["file1.svs"]}).to_csv(
            invalid_csv_path, index=False
        )
        with self.assertRaises(
            KeyError, msg="Should raise KeyError if 'Filename' column is missing"
        ):
...
class TestCloseLogger(unittest.TestCase):
    ...
    def test_close_logger_no_handlers(self):
        ...
        close_logger(logger)
        self.assertEqual(
            len(logger.handlers), 0, "Logger should have no handlers after closing"
        )
    ...
    def test_close_logger_with_handlers(self):
        ...
        self.assertEqual(
            len(logger.handlers), 1, "Logger should have one handler before closing"
        )
        close_logger(logger)
        self.assertEqual(
            len(logger.handlers), 0, "Logger should have no handlers after closing"
    ...
    def test_close_logger_multiple_handlers(self):
        ...
        self.assertEqual(
            len(logger.handlers), 2, "Logger should have two handlers before closing"
        )
        close_logger(logger)
        self.assertEqual(
            len(logger.handlers), 0, "Logger should have no handlers after closing"
    ...
    def test_close_logger_shutdown(self):
        ...
        with patch(
            "logging.shutdown"
        ) as mock_shutdown:  # Verwende das bereits importierte patch
            close_logger(logger)
...
class TestRemoveParameterTag(unittest.TestCase):
    ...
    def test_remove_parameter_tag_with_parameters_key(self):
        ...
        self.assertEqual(
            result, expected, "Keys containing 'parameters' should be removed correctly"
        )
    ...
    def test_remove_parameter_tag_custom_separator(self):
        ...
        expected = {"a": {"b": 1}, "x/y": {"z": 2}}
        self.assertEqual(
            result, expected, "Custom separator should be handled correctly"
        )
    ...
    def test_remove_parameter_tag_no_removal_needed(self):
        ...
        expected = {"a.b": {"c": 1}, "x.y": {"z": 2}}
        self.assertEqual(
            result, expected, "Dictionary without 'parameters' should remain unchanged"
        )
...
class TestRemapLabel(unittest.TestCase):
    ...
    def test_remap_label_no_instances(self):
        ...
        np.testing.assert_array_equal(
            result, pred, "Output should match input when there are no instances"
        )
    ...
    def test_remap_label_single_instance(self):
        ...
        expected[1:4, 1:4] = 1
        np.testing.assert_array_equal(
            result, expected, "Single instance should remain unchanged"
        )
    ...
    def test_remap_label_multiple_instances(self):
        ...
        expected[3:5, 3:5] = 2
        np.testing.assert_array_equal(
            result, expected, "Instances should be renumbered contiguously"
        )
    ...
    def test_remap_label_by_size(self):
        ...
        expected[3:5, 3:6] = 1
        np.testing.assert_array_equal(
            result, expected, "Instances should be renumbered by size"
        )
    ...
    def test_remap_label_with_zero_background(self):
        ...
        expected[3:5, 3:5] = 2
        np.testing.assert_array_equal(
            result, expected, "Zero background should remain unchanged"
        )
    ...
    def test_remap_label_all_removed(self):
        ...
        expected = np.zeros((5, 5), dtype=int)
        np.testing.assert_array_equal(
            result, expected, "Output should be all zeros when input has no instances"
        )
