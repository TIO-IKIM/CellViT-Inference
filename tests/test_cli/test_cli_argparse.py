...
class TestInferenceWSIParser(unittest.TestCase):
    ...
    def test_parse_wsi_arguments(self, mock_device_count, mock_parse_args):
        ...
        self.assertEqual(
            config.command, "process_wsi", "Command should be 'process_wsi'"
        )
        self.assertEqual(
            config.wsi_path,
    ...
    def test_parse_dataset_arguments(self, mock_device_count, mock_parse_args):
        ...
        self.assertEqual(config.model, "SAM", "Model should be SAM")
        self.assertEqual(
            config.command, "process_dataset", "Command should be 'process_dataset'"
        )
        self.assertEqual(
            config.wsi_folder,
    ...
    def test_transform_cli_to_yaml(self):
        ...
        self.assertTrue(
            yaml_config["inference"]["enforce_amp"], "Enforce AMP should be True"
        )
        self.assertEqual(
            yaml_config["inference"]["batch_size"], 8, "Batch size should be 8"
        )
        self.assertEqual(
        ...
        self.assertTrue(
            yaml_config["output_format"]["geojson"], "GeoJSON should be True"
        )
        self.assertFalse(yaml_config["output_format"]["graph"], "Graph should be False")
        self.assertTrue(
            yaml_config["output_format"]["compression"], "Compression should be True"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            yaml_config["process_wsi"]["wsi_mpp"], 0.25, "WSI MPP should be 0.25"
        )
        self.assertEqual(
    ...
    def test_cli_help_message(self, mock_device_count, mock_parse_args):
        ...
        with patch("sys.stdout", new=io.StringIO()) as fake_stdout:
            ...
            self.assertIn(
                "CellViT++", output, "Help message should contain 'CellViT++'"
            )
